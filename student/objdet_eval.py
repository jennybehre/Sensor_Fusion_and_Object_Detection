...
def measure_detection_performance(detections, labels, labels_valid, min_iou=0.5):
    ...
    for label, valid in zip(labels, labels_valid):
        ...
        if valid: # exclude all labels from statistics which are not considered valid
            ...
            # See Udacity Knowledge for references
            ## step 1 : extract the four corners of the current label bounding-box
            label_bbox = label.box
            box_1 = tools.compute_box_corners(label_bbox.center_x, label_bbox.center_y, label_bbox.width, label_bbox.length, label_bbox.heading)
            
            ## step 2 : loop over all detected objects
            for detect_bbox in detections:
                
                ## step 3 : extract the four corners of the current detection
                _id, x, y, z, _h, w, l, yaw = detect_bbox
